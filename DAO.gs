...
/**
 * Invalida o cache do mapa de uma aba.
 * Chamar sempre que adicionar, remover ou renomear colunas na planilha.
 */
function invalidateHeaderCache(sheetName) {
  const cache = CacheService.getScriptCache();
  cache.remove("hmap_"  + sheetName); // chave legada
  cache.remove("hmap2_" + sheetName); // chave atual
}
...
function fetchAll(sheetName) {
  ...
  return rows.map((row, idx) => {
    const obj = { __rowNumber: idx + 2 }; // índice real na planilha (linha 1 = cabeçalho)
    keys.forEach(key => {
